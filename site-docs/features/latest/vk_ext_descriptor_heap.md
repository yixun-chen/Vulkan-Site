...
These addresses can also be mapped to an existing uniform buffer declaration in the shader using [`DescriptorSet` and `Binding` Decorations](#SetAndBindingDecorations), which will be the preferred path for some implementations initially, though such implementations are expected to lean less on this mechanism over time. |
